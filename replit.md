...
- **Professional Movement Modal Redesign**: Complete UI/UX redesign of movement creation/editing modal with dynamic sections based on income/expense selection. Three main sections: Base Information (dates, company, core, reason, type), Entity Association (customer/supplier + resource/office with detailed cards), and Financial Details (IBAN, status, amounts, VAT, documents). Added customer field to database schema, data table columns, and filters. Professional styling with section cards, entity preview cards, automatic VAT calculation, and XML invoice integration. (August 13, 2025)
